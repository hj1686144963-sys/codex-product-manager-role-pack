...
请安装当前文件夹中的「Codex 产品经理公共能力包 0.2.0」。按下面顺序执行，不要让我手动选择 Agent、Skill、推理强度或知识库目录：
...
5. 安装五个角色和四个 Skill：`product-manager-core`、`five-role-deliberation`、`codex-auto-memory`、`leiniao-ui-design-baseline`。
...
8. 运行 `python3 scripts/build_knowledge_index.py --root assets/knowledge-base`、`python3 scripts/run_role_eval.py` 和 `python3 scripts/validate_skill_registry.py`。
