...
def install_role_assets(plugin_root: Path, codex: Path, home: Path) -> None:
    """Install the central, product, review, and support role files plus bundled skills."""
...
def main() -> int:
    ...
    try:
        ...
        behavior_target = vault / "02-Knowledge" / "产品经理行为记忆.md"
        if not behavior_target.exists():
            behavior_target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(behavior_source, behavior_target)
            state["created_vault_files"].append("02-Knowledge/产品经理行为记忆.md")
    ...
    result = {
        ...
        "next": [
            ...
            "Start a new chat; the central Agent automatically routes the primary job, product support roles, or shared five-role deliberation.",
        ],
    }
