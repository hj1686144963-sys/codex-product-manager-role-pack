#!/usr/bin/env python3
"""Verify an installed Codex product-manager role pack."""
...
DESIGN_SKILL = "product-ui-design-baseline"
...
def main() -> int:
    ...
    checks.append(
        check(
            "runtime_version",
            runtime.get("plugin_version") == PLUGIN_VERSION,
            str(runtime.get("plugin_version", "missing")),
        )
    )
    checks.append(check("plugin_manifest", (plugin_root / ".codex-plugin" / "plugin.json").is_file(), str(plugin_root)))

    role_names = [
        "product-manager-specialist.toml",
        "ui-designer-specialist.toml",
        "ai-agent-architect-specialist.toml",
        "director-specialist.toml",
        "coordinator.toml",
        "researcher.toml",
        "product-strategist.toml",
        "system-strategist.toml",
        "red-team-editor.toml",
    ]
    role_root = codex / "agents"
    checks.append(
        check(
            "role_agents",
            all((role_root / name).is_file() for name in role_names),
            ", ".join(role_names),
        )
    )
    deliberation_skill = home / ".agents" / "skills" / FIVE_ROLE_SKILL / "SKILL.md"
    checks.append(check("five_role_skill", deliberation_skill.is_file(), str(deliberation_skill)))
    design_skill = codex / "skills" / DESIGN_SKILL / "SKILL.md"
    design_reference = codex / "skills" / DESIGN_SKILL / "references" / "ui-visual-design-baseline.md"
    checks.append(
        check(
            "design_baseline_skill",
            design_skill.is_file() and design_reference.is_file(),
            f"{design_skill}; {design_reference}",
        )
    )
    auto_memory_skill = codex / "skills" / AUTO_MEMORY_SKILL / "SKILL.md"
    product_manager_skill = codex / "skills" / PRODUCT_MANAGER_SKILL / "SKILL.md"
    checks.append(check("auto_memory_skill", auto_memory_skill.is_file(), str(auto_memory_skill)))
    checks.append(check("product_manager_skill", product_manager_skill.is_file(), str(product_manager_skill)))
    ...
    if vault_ok:
        required = [
            ...
            "AGENTS.md",
            "01-Role/产品经理岗位画像.md",
            "04-Methods/产品UI设计基线.md",
            "02-Knowledge/产品经理行为记忆.md",
            "08-System/决策记录.md",
            "08-System/防踩坑记录.md",
        ]
        checks.append(
