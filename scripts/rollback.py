...
def main() -> int:
    ...
    targets = {
        ...
        "five_role_skill": (backup / "five-role-skill", Path(paths["Five-role skill"])),
        "design_skill": (backup / "design-skill", Path(paths["Design baseline skill"])),
        "auto_memory_skill": (backup / "auto-memory-skill", Path(paths["Auto-memory skill"])),
        "product_manager_skill": (backup / "product-manager-skill", Path(paths["Product-manager skill"])),
    }
    role_destination = Path(paths.get("Role agents", paths.get("Five-role agents")))
    for name, was_present in existed.items():
        if not name.startswith("role:"):
            continue
        role_name = name.split(":", 1)[1]
        targets[name] = (backup / "agents" / role_name, role_destination / role_name)
